        ...
        route("/api/products") {
            get {
                ...
                val items = service.listProducts(
                    ProductFilter(
                        category = call.request.queryParameters["category"]?.let { ProductCategory.valueOf(it) },
                        cookingRequirement = call.request.queryParameters["cookingRequirement"]?.let { CookingRequirement.valueOf(it) },
                        flags = flags,
        ...
        route("/api/dishes") {
            get {
                ...
                call.respond(
                    service.listDishes(
                        DishFilter(
                            category = call.request.queryParameters["category"]?.let { DishCategory.valueOf(it) },
                            flags = flags,
                            query = call.request.queryParameters["query"],
                        )
                    )
